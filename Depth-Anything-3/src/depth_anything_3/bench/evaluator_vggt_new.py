...
if __name__ == "__main__":
    ...
    if cli_args.lq_root_path:
        constants.DA3_LQ_ROOT_PATH = cli_args.lq_root_path

    if cli_args.res_root_path:
        constants.DA3_RES_ROOT_PATH = cli_args.res_root_path
    ...
    if "--config" in argv:
        ...
        raise ValueError("Please provide a config file using --config PATH")
...
Examples:
  ...
  python -m depth_anything_3.bench.evaluator eval.print_only=true
    ...
    work_dir = cli_args.work_dir if cli_args.work_dir else config.workspace.work_dir
    ...
    max_frames = cli_args.max_frames if cli_args.max_frames else config.eval.max_frames
    ...
    if print_only:
        ...
        if len(gpu_list) > 1 and not is_worker:
            ...
            from vggt.vggt.utils.api_new import VGGTInference
