...
def main():
    ...
    if len(data_cfg["train"]["dataset"]) == 1:
        sample_img = train_loader.dataset[0]["deg_img"].to(device) # [3, H, W]
        vggt_patch_size = model_config["params"].get("vggt_patch_size", 14)
        pH, pW = sample_img.shape[1] // vggt_patch_size, sample_img.shape[2] // vggt_patch_size
    ...
    for epoch in range(start_epoch, num_epochs): # epoch: 1400
        ...
        for step, batch in enumerate(train_loader):
            clean_img = batch["clean_img"] # [B, 3, 392, 518]
            deg_img = batch["deg_img"]     # [B, 3, 392, 518]
            gt_depth = batch["gt_depth"]   # [B, 1, 392, 518]
            ...
            with torch.no_grad():
                ...
                #     continue
                # breakpoint()
                clean_predictions = vggt_model(clean_img.to(device).float(), extract_layer_num=3) # [B, 1, 1041, 2048]
                ...
                predictions = vggt_model(deg_img.to(device).float(), extract_layer_num=3) # [B, 1, 1041, 2048]
            ...
            if global_step % sample_every == 0:
                ...
                with torch.no_grad():
                    if len(deg_latent.shape) == 4:
                        deg_latent = deg_latent.squeeze(1) # [B, 1041, 1024]

                    zs = torch.randn(*deg_latent.shape, device=device, dtype=torch.float32) # [B, 1041, 1024]
                    assert zs.shape == deg_latent.shape, f"Latent shape mismatch: {zs.shape} vs {deg_latent.shape}" # [B, 1041, 1024]
                    
                    val_xt = zs + deg_latent # [B, 1041, 1024]
                    # clean_img_latent = clean_img_latent[:, 1024:, :, :]
                    # val_xt = zs + clean_img_latent        
                    ...
                    val_predictions = vggt_model(deg_img.to(device).float(), extract_layer_num=3, vggt_result=vggt_result, change_latent=True)
                    # val_predictions = vggt_model(clean_img.to(device).float(), extract_layer_num=3, vggt_result=vggt_result, change_latent=True)
                    # val_predictions['extracted_latent']: [B, 1, 1041, 2048]
                    val_depths = val_predictions['depth'] # [B, 1, 392, 518, 1]
                    breakpoint()
                    train_metric = compute_depth_metrics(gt_depth, train_depths)
                    val_metric = compute_depth_metrics(gt_depth, val_depths)

                    if rank == 0:
                        metric_csv_logger.log(global_step, train_metric, val_metric)
                        
                        logger.info(f"Step {global_step}: Metrics saved to CSV.")
                        logger.info(f"Train AbsRel: {train_metric['abs_rel']:.4f} | Val AbsRel: {val_metric['abs_rel']:.4f}")
                    ...
                    train_error_vis = process_depth_error_batch(gt_depth, train_depths) # [B, H, W, 3]
                    val_error_vis = process_depth_error_batch(gt_depth, val_depths)     # [B, H, W, 3]
                    ...
                    idx = 0
                    imgs_list = [
                        to_rgb_tensor(clean_img[idx:idx+1]),            # 1. GT RGB
                        to_rgb_tensor(deg_img[idx:idx+1]),              # 2. Deg RGB
                        to_rgb_tensor(gt_depth[idx:idx+1], True),       # 3. GT Depth
                        to_rgb_tensor(clean_latent_depths[idx:idx+1], True), # 4. Clean Latent Depth
                        to_rgb_tensor(train_depths[idx:idx+1], True),   # 5. Deg Depth (Input)
                        error_to_tensor(train_error_vis[idx:idx+1]),    # 6. Input Error
                        to_rgb_tensor(val_depths[idx:idx+1], True),     # 7. Restored Depth (Output)
                        error_to_tensor(val_error_vis[idx:idx+1])       # 8. Output Error
                    ]

                    from torchvision.utils import make_grid
                    grid = make_grid(torch.cat(imgs_list, dim=0), nrow=4, padding=5, pad_value=1.0) 

                    vis_dir = f"{experiment_dir}/visualizations"
                    os.makedirs(vis_dir, exist_ok=True)
                    save_path = f"{vis_dir}/depth_step{global_step:07d}.png"
                    save_image(grid, save_path, normalize=False)
                    
                    # save depth map
                    # vis_dir = f"{experiment_dir}/vis_depth"
                    # os.makedirs(vis_dir, exist_ok=True)

                    # # 1. GT Depth (B, 1, H, W)
                    # gt_depth_vis = process_depth_batch(gt_depth) # [B, H, W, 3]
                    # gt_depth_tensor = torch.from_numpy(gt_depth_vis).permute(0, 3, 1, 2).float() / 255.0 # [B, 3, H, W]
                    # save_image(gt_depth_tensor[0], f"{vis_dir}/gt_depth_step{global_step}.png")

                    # # 2. Input Degraded Depth (B, 1, H, W, 1) 
                    # train_depth_vis = process_depth_batch(train_depths) # [B, H, W, 3]
                    # train_depth_tensor = torch.from_numpy(train_depth_vis).permute(0, 3, 1, 2).float() / 255.0 # [B, 3, H, W]
                    # save_image(train_depth_tensor[0], f"{vis_dir}/input_deg_depth_step{global_step}.png")

                    # # 3. Restored/Sampled Depth (B, 1, H, W, 1)
                    # val_depth_vis = process_depth_batch(val_depths) # [B, H, W, 3]
                    # val_depth_tensor = torch.from_numpy(val_depth_vis).permute(0, 3, 1, 2).float() / 255.0 # [B, 3, H, W]
                    # save_image(val_depth_tensor[0], f"{vis_dir}/sampled_depth_step{global_step}.png")

                logger.info("Sampling done.")
                logger.info("Starting Error Map Visualization...")
                with torch.no_grad():
                    latent_error_vis = visualize_latent_error_map(deg_img, clean_img_latent, restored_latent) # [B, 1, 392, 518]
                    ...
                    idx = 0
                    imgs_list = [
                        to_rgb_tensor(clean_img[idx:idx+1]).cpu(),            # 1. GT RGB
                        to_rgb_tensor(deg_img[idx:idx+1]).cpu(),              # 2. Deg RGB
                        to_rgb_tensor(gt_depth[idx:idx+1], True).cpu(),       # 3. GT Depth
                        to_rgb_tensor(val_depths[idx:idx+1], True).cpu(),     # 4. Restored Depth
                        error_to_tensor(val_error_vis[idx:idx+1]).cpu(),      # 5. Depth Error Map (Red)
                        latent_error_vis[idx:idx+1].repeat(1, 3, 1, 1).cpu()  # 6. Latent Error Map (Gray/Heatmap)
                    ]

                    grid = make_grid(torch.cat(imgs_list, dim=0), nrow=3, padding=5, pad_value=1.0)
                    error_vis_dir = f"{experiment_dir}/error_vis"
                    os.makedirs(error_vis_dir, exist_ok=True)
                    save_image(grid, f"{error_vis_dir}/analysis_step{global_step:07d}.png")
                logger.info("Error Map Visualization done.")
                model.train()
