...
from utils.train_utils_JIHYE import *
from utils.optim_utils import build_optimizer, build_scheduler
from utils.resume_utils_JIHYE import *
...
def main():
    ...
    train_loader, train_sampler = load_dataloader(
        data_cfg, micro_batch_size, num_workers, rank, world_size, mode='train', max_view=args.max_view, kernel_size=args.kernel_size
    )

    # val_loader, val_sampler = load_dataloader(
    #     data_cfg, micro_batch_size, num_workers, rank, world_size, mode='val', max_view=args.max_view, kernel_size=args.kernel_size
    # )
    ...
    for epoch in range(start_epoch, num_epochs): # epoch: 1400
        ...
        for step, batch in enumerate(train_loader):
            ...
            if global_step % sample_every == 0 and rank == 0 and val_loader is not None:
                ...
                for val_step, val_batch in enumerate(tqdm(val_loader, desc="Validation")):
                    ...
                    with torch.no_grad():
                        ...
                        if val_step < 5:
                            ...
                            os.makedirs(vis_dir, exist_ok=True)
                            save_path = f"{vis_dir}/val_multiview_step{global_step:07d}_{val_step}.png"
                            save_image(grid, save_path, normalize=False)
