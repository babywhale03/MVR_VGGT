...
class DiTwDDTHead(nn.Module):
    def __init__(
        ...
        self.num_blocks = sum(depth)
        
        self.input_proj = nn.Linear(in_channels, self.encoder_hidden_size)

        self.t_embedder = GaussianFourierEmbedding(self.encoder_hidden_size)
        # self.y_embedder = LabelEmbedder(num_classes, self.encoder_hidden_size, 0.1)

        self.use_pos_embed = use_pos_embed
        if use_pos_embed:
            self.pos_embed = nn.Parameter(
                torch.zeros(1, 1041, self.encoder_hidden_size), requires_grad=False
            )

        self.use_rope = use_rope
        if use_rope:
            h_p, w_p = 28, 37 # 392/14, 518/14
            enc_half = self.encoder_hidden_size // num_heads[0] // 2
            self.enc_feat_rope = VisionRotaryEmbeddingFast(dim=enc_half, pt_seq_len=(h_p, w_p))
            dec_half = self.decoder_hidden_size // num_heads[1] // 2
            self.dec_feat_rope = VisionRotaryEmbeddingFast(dim=dec_half, pt_seq_len=(h_p, w_p))

        self.blocks = nn.ModuleList([
            LightningDDTBlock(
                hidden_size=self.encoder_hidden_size if i < self.num_encoder_blocks else self.decoder_hidden_size,
                num_heads=num_heads[0] if i < self.num_encoder_blocks else num_heads[1],
                mlp_ratio=mlp_ratio,
            ) for i in range(self.num_blocks)
        ])

        self.s_projector = nn.Linear(self.encoder_hidden_size, self.decoder_hidden_size)
        
        self.final_layer = DDTFinalLayer(self.decoder_hidden_size, 1, 2048)
    ...
    def initialize_weights(self):
        if self.use_pos_embed:
            grid_h, grid_w = 28, 37
            pos_embed_content = get_2d_sincos_pos_embed(self.encoder_hidden_size, (grid_h, grid_w))
            self.pos_embed.data[:, 5:, :].copy_(torch.from_numpy(pos_embed_content).float().unsqueeze(0))
    ...
    def forward(self, x, t):
        breakpoint()
        x = self.input_proj(x) # [B, 1041, 1152]
        ...
        s_for_dec = self.s_projector(s) # [B, 1041, 2048]
        x_dec = s_for_dec
        
        for i in range(self.num_encoder_blocks, self.num_blocks):
            x_dec = self.blocks[i](x_dec, s_for_dec, feat_rope=self.dec_feat_rope)

        x_out = self.final_layer(x_dec, s_for_dec) # [B, 1041, 4096]
...
#                 pt_seq_len=(h_patches, w_patches),
#             )
...
#     def forward(self, x, t, s=None, mask=None):
#         breakpoint()
#         # x: [32, 1041, 4096], t: [32]
#         # x = self.x_embedder(x) + self.pos_embed
#         t = self.t_embedder(t) # [32, 4096]
#         # y = self.y_embedder(y, self.training) # [32, 1152]
...
#         s = self.s_projector(s) # linear layer
#         x = self.x_embedder(x)
