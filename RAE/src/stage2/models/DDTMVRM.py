...
class DiTwDDTHead(nn.Module):
    def __init__(
        ...
        self.x_embedder = PatchEmbed(
            ...
            in_chans=x_channel_per_token, # 4096
            embed_dim=self.decoder_hidden_size, # 2048
            bias=True
        )
        self.s_embedder = PatchEmbed(
            img_size=(28, 37),
            patch_size=self.s_patch_size,
            in_chans=in_channels, # 4096
            embed_dim=self.encoder_hidden_size, # 1152
            bias=True
        )
        ...
        self.final_layer = DDTFinalLayer(
            self.decoder_hidden_size, 1, 1024, use_rmsnorm=use_rmsnorm)
        ...
        if self.use_rope:
            enc_half_head_dim = self.encoder_hidden_size // enc_num_heads // 2 # 1152 // 16 // 2 = 36
            ...
            dec_half_head_dim = self.decoder_hidden_size // dec_num_heads // 2 # 2048 // 16 // 2 = 64
    ...
    def initialize_weights(self, xavier_uniform_init: bool = False):
        ...
        if self.use_pos_embed:
            ...
            pH, pW = 28, 37  # 392/14, 518/14
            D = self.pos_embed.shape[-1]
            pos_embed = get_2d_sincos_pos_embed_rect(D, pH, pW)
    ...
    def unpatchify(self, x):
        ...
        h = 28
        w = 37
        assert h * w == N, f"{h}×{w} != {N}"

        x = x.reshape(B, h, w, p, p, c)
        x = x.permute(0, 5, 1, 3, 2, 4)   # BCHPWQ
    ...
    def forward(self, x, t, s=None, mask=None):
        ...
        breakpoint()
        t = self.t_embedder(t) # [32, 1152]? 
        # y = self.y_embedder(y, self.training) # [32, 1152]
        c = nn.functional.silu(t)
        if s is None:
            s = self.s_embedder(x) # [32, 4096, 28, 37] -> [32, 1036, 1152]
            if self.use_pos_embed:
                s = s + self.pos_embed 
            ...
            # if s.dim() > 3:
            #     s = s.squeeze(1)
        s = self.s_projector(s) # linear layer [32, 1036, 2048]?
        x = self.x_embedder(x) # [32, 1036, 2048]
