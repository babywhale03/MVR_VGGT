...
class DiTwDDTHead(nn.Module):
    ...
    def forward(self, clean_img, x, t, residual=False, step=None, experiment_dir=None, s=None, mask=None):
        ...
        c = nn.functional.silu(t).view(B, 1, 1, -1) # [B, 1, 1, C]
        ...
        if s is None:
            ...
            for i in range(self.num_encoder_blocks): # num_encoder_blocks=28
                block = self.blocks[i]
                if i % 2 == 0:
                    s = self.process_frame_attention(s, c, block, pos=pos) # c: [B, 1, 1, C]
                else:
                    s = self.process_global_attention(s, c, block, pos=pos) # [B, S * P, C]

            if len(s.shape) != 4:
                s = s.view(B, S, P, C)
            t = t.view(B, 1, 1, -1).repeat(1, S, P, 1) # [B, S, P, C]
            s = nn.functional.silu(t + s)

        s = self.s_projector(s) # linear layer [B, S, 1041, 2048]
