...
def get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False):
    ...
    return pos_embed # [H*W, D] or [1+H*W, D]
...
class VisionRotaryEmbeddingFast(nn.Module):
    def __init__(
        ...
        if ft_seq_len is None:
            ...
            h_ft, w_ft = ft_seq_len, ft_seq_len
        else:
            h_ft, w_ft = ft_seq_len

        t_h = torch.arange(h_ft) / h_ft * h_pt
        t_w = torch.arange(w_ft) / w_ft * w_pt

        h_freqs = torch.einsum('i, f -> i f', t_h, freqs)
        w_freqs = torch.einsum('j, f -> j f', t_w, freqs)
        ...
        combined_freqs = torch.cat([
            h_grid.expand(h_ft, w_ft, -1),
            w_grid.expand(h_ft, w_ft, -1)
        ], dim=-1) # [H, W, dim * 2]

        freqs_cos = combined_freqs.cos().view(-1, combined_freqs.shape[-1]) # [H*W, dim*2]
        freqs_sin = combined_freqs.sin().view(-1, combined_freqs.shape[-1]) # [H*W, dim*2]

        self.register_buffer("freqs_cos", freqs_cos)
        self.register_buffer("freqs_sin", freqs_sin)

    def forward(self, t):
        # _, _, Lt, dim_total = t.shape
        # L_grid, _ = self.freqs_cos.shape
        # if Lt > L_grid:
        #     diff = Lt - L_grid
        #     pad_cos = torch.ones(diff, dim_total, device=t.device, dtype=t.dtype)
        #     pad_sin = torch.zeros(diff, dim_total, device=t.device, dtype=t.dtype)
            
        #     f_cos = torch.cat([pad_cos, self.freqs_cos], dim=0)
        #     f_sin = torch.cat([pad_sin, self.freqs_sin], dim=0)
        # else:
        #     f_cos, f_sin = self.freqs_cos, self.freqs_sin

        # return t * f_cos + rotate_half(t) * f_sin
...
class JaeAttention(nn.Module):
    """
    Attention Module with VGGT Style RoPE Embedding
    """
    def __init__(
        self,
        dim: int,
    ...
    def forward(self, x: torch.Tensor, pos=None, return_attn: bool = False) -> torch.Tensor:
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
        q, k = self.q_norm(q), self.k_norm(k)
        ...
        if self.fused_attn and not return_attn:
            ...
            attn = attn.softmax(dim=-1)
            attn_map = attn.detach() # [B, num_heads, N, N]
            attn = self.attn_drop(attn)
            x = attn @ v
