...
def parse_configs(config: Union[DictConfig, str]) -> Tuple[DictConfig, DictConfig, DictConfig, DictConfig, DictConfig, DictConfig, DictConfig]:
    ...
    return stage2_config, transport_config, sampler_config, guidance_config, misc, training_config, data_config, log_config, eval_config
...
class HypersimDataset(Dataset):
    def __init__(self, clean_img_paths: str, gt_depth_paths: str, annotation_path: str = None, mode: str = 'train', num_eval_img: int = 100, max_view: int = 8, kernel_size: int = 100, view_sel: dict = None):
        self.mode = mode
        self.num_eval_img = num_eval_img
        self.num_view = max_view
        self.view_sel = view_sel
        self.curr_batch_num_view = max_view
        ...
        for h, ann in zip(all_hq, target_annotations):
            if os.path.exists(h):
    ...
    def __getitem__(self, item):
        ...
        return {
            ...
            "gt_depth": torch.zeros_like(torch.stack(clean_imgs)[:, :1, :, :]),   # [curr_num_view, 1, 518, 518]
...
class TartanAirDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        last_valid_clean, last_valid_depth = None, None
        ...
        for i in indices:
            try:
                ...
                last_valid_clean = c_t
                last_valid_depth = d_t

            except Exception as e:
                print(f"[Warning] Failed to load frame {self.clean_img_paths[i]}: {e}")
                if last_valid_clean is not None:
                    c_t, d_t = last_valid_clean.clone(), last_valid_depth.clone()
                else:
                    ...
                    last_valid_clean, last_valid_depth = c_t.clone(), d_t.clone()
