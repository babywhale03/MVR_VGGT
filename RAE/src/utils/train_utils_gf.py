...
class HypersimDataset_pre(Dataset):
    def __init__(self, clean_img_paths: str, gt_depth_paths: str, annotation_path: str = None, mode: str = 'train', num_eval_img: int = 100, num_view: int = 1, view_sel: dict = None, kernel_size: int = 100):
        ...
        for scene_id, camera_id in sorted(common_keys):
            ...
            self.camera_extrinsics_data[cache_key] = extrinsics
    ...
    def __getitem__(self, item):
        ...
        deg_imgs, clean_imgs, gt_depths, gt_poses_list = [], [], [], []
        valid_indices = []
        ...
        for i in indices:
            ...
            scene_id, camera_id, frame_id = self.global_to_camera_idx[i]
            cache_key = (scene_id, camera_id) 
            all_poses = self.camera_extrinsics_data.get(cache_key)
            
            try:
                clean_t, deg_t = process_clean_deg_tensors_from_hdf5(cp, kernel=self.kernel, target_size=518, mode="crop")
                depth_t = process_single_depth_hdf5(dp, target_size=518, mode="crop")
                
                if all_poses is not None and frame_id < len(all_poses):
                    c2w = torch.from_numpy(all_poses[frame_id]).float()
                    w2c = torch.inverse(c2w) 
                    gt_poses_list.append(w2c)
                else:
                    gt_poses_list.append(torch.eye(4))
        ...
        return {
            ...
            "gt_poses": torch.stack(gt_poses_list), # [curr_num_view, 4, 4] 
            "indices": torch.tensor(valid_indices),
        }
    
class TartanAirDataset_pre(Dataset):
    def __init__(self, clean_img_paths: str, gt_depth_paths: str, annotation_path: str = None, mode: str = 'train', num_eval_img: int = 100, num_view: int = 1, view_sel: dict = None, kernel_size: int = 100):
        self.mode = mode
        self.num_view = num_view
        self.view_sel = view_sel
        ...
        unique_scenes = set(self.scene_names)
        for scene_path in unique_scenes:
            pose_file = os.path.join(scene_path, "pose_left.txt")
            if os.path.exists(pose_file):
                self.pose_data[scene_path] = np.loadtxt(pose_file)

        print(f"--- TartanAir Dataset ({mode}) ---")
        print(f"Number of clean images: {len(self.clean_img_paths)}")
        print(f"Number of gt depth maps: {len(self.gt_depth_paths)}")

    def tartanair_pose_to_matrix(pose_vec):
        """
        input: [x, y, z, qx, qy, qz, qw] (1, 7)
        output: 4x4 Transformation Matrix (C2W)
        """
        t = pose_vec[:3]
        q = pose_vec[3:] 
    ...
    def __getitem__(self, item):
        ...
        current_scene_path = self.scene_names[actual_idx]
        all_poses = self.pose_data.get(current_scene_path)

        for i in indices:
            scene_start_idx = self.scene_to_indices[current_scene_path][0]
            relative_idx = i - scene_start_idx

            if all_poses is not None and relative_idx < len(all_poses):
                c2w = self.tartanair_pose_to_matrix(all_poses[relative_idx])
                w2c = np.linalg.inv(c2w)
                pose_list.append(torch.from_numpy(w2c))
            else:
        ...
        return {
            ...
            "gt_poses": torch.stack(pose_list), 
...
class ETH3DDataset(Dataset):
    ...
    def _parse_colmap_images(self, clean_img_paths):
        ...
        for scene in os.listdir(clean_img_paths):
            img_txt_path = os.path.join(clean_img_paths, scene, "dslr_calibration_jpg", "images.txt")
