...
class PoseMetricTracker:
    ...
    def update(self, r_err, t_err):
        # r_err, t_err: se3_to_relative_pose_error에서 나온 numpy array들
    ...
    if use_ba:
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(dtype=dtype):
                predictions = model(images)
        with torch.cuda.amp.autocast(dtype=torch.float64):
            extrinsic, intrinsic = pose_encoding_to_extri_intri(predictions["pose_enc"], images.shape[-2:]) # [10, 1, 3, 4], [10, 1, 3, 3]
            # pred_extrinsic = extrinsic[0]
            pred_extrinsic = extrinsic[:, 0, :, :] # [10, 3, 4]
...
def main():
    ...
    log_dir = os.path.join("eval_logs", "co3d_eval")
    ...
    for category in SEEN_CATEGORIES:
        ...
        category_dir = os.path.join(args.co3d_dir, category)
        if not os.path.exists(category_dir):
            logger.info(f"Category folder {category_dir} not found, skipping.")
            continue
        ...
        for seq_name in seq_names:
            ...
            if not os.path.exists(seq_dir):
                logger.info(f"Sequence folder {seq_dir} not found, skipping.")
                continue
        ...
        if not rError:
            logger.warning(f"No valid sequences found for {category}, skipping")
            continue
