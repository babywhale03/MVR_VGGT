...
from RAE.src.utils.vis_utils import process_depth_batch
...
def process_sequence(model, eval_sampler, seq_name, seq_data, category, co3d_dir, min_num_images, num_frames, use_ba, device, dtype, logger, save_dir):
    """
    ...
    if use_ba:
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(dtype=dtype):
                ...
                val_lq_depths = val_lq_predictions["depth"] # [B, 1, 1, H, W]
                ...
                val_depths = val_predictions["depth"] 

                B, V, C, H, W = deg_images.shape
                
                v_clean = clean_images[0]           # [V, 3, H, W]
                v_deg = deg_images[0]               # [V, 3, H, W]
                
                v_lq_depth = val_lq_depths[0].squeeze(-1)    # [V, 1, H, W]
                v_restored_depth = val_depths[0].squeeze(-1) # [V, 1, H, W]
                ...
                for j in range(V):
                    for element in row_elements:
                        combined_rows.append(element[j:j+1]) 
                
                all_vis_tensors = torch.cat(combined_rows, dim=0)

                grid = make_grid(all_vis_tensors, nrow=4, padding=10, pad_value=1.0) 

                save_image(grid, os.path.join(save_dir, f"depth_vis.png"), normalize=False)
    ...
    with torch.cuda.amp.autocast(dtype=torch.float64):
        ...
        plot_trajectory(gt_se3_aligned, pred_se3_aligned, lq_se3_aligned, seq_name, save_dir)

        return rel_rangle_deg.cpu().numpy(), rel_tangle_deg.cpu().numpy(), lq_rel_rangle_deg.cpu().numpy(), lq_rel_tangle_deg.cpu().numpy()

def evaluate_co3d(model, eval_sampler, logger, co3d_dir, co3d_anno_dir, save_dir, min_num_images=50, num_frames=10, use_ba=False):
    ...
    for category in categories:
        ...
        for seq_name in seq_names:
            ...
            seq_rError, seq_tError, lq_seq_rError, lq_seq_tError = process_sequence(
                ...
                min_num_images, num_frames, use_ba, device, dtype, logger, save_dir
            )
