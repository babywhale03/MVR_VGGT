...
class RotaryPositionEmbedding2D(nn.Module):
    ...
    ### 속도 최적화 위해 바꿔본 것
    # def _get_cos_sin_directly(self, positions: torch.Tensor, dim: int, device: torch.device, dtype: torch.dtype):
    ...
    #     cos_v, sin_v = self._get_cos_sin_directly(positions[..., 0], feature_dim, tokens.device, tokens.dtype)
    #     cos_h, sin_h = self._get_cos_sin_directly(positions[..., 1], feature_dim, tokens.device, tokens.dtype)

    #     v_feat, h_feat = tokens.chunk(2, dim=-1)

    #     v_feat = (v_feat * cos_v) + (self._rotate_features(v_feat) * sin_v)
    #     h_feat = (h_feat * cos_h) + (self._rotate_features(h_feat) * sin_h)
